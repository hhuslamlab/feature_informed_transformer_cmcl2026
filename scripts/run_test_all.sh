...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${BASE_DIR}/example/transformer/checkpoints/transformer/tagtransformer}"
...
OUTPUT_DIR="${OUTPUT_DIR:-predictions_nevins/feature_invariant}"

# Common args
COMMON_ARGS="--eval_test --arch tagtransformer --dataset taginbrackets --bs 16 --decode beam --decode_beam_size 5"
...
for condition in 10L_90NL 50L_50NL 90L_10NL; do
    ...
    for run in 1 2 3; do
        for seed in 1 2 3 4; do
            ...
            # Find checkpoint with epoch_103 (exclude .log files)
            CHECKPOINT=$(find "${CHECKPOINT_DIR}" -name "*${MODEL_NAME}*epoch_103*" ! -name "*.log" -type f 2>/dev/null | head -1)
